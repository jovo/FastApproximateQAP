...
figure(2); clf
subplot(121) %'Position',[left bottom1 width height]);
boxplot(allErrors(algsKept,:,1)'...
    ,'labels',{'FAQ','U','QCV','PATH'}...
    ,'labelorientation','inline'...
    ,'color','k','symbol','k+','plotstyle','compact'); %...
ylabel('error (%)','fontsize',fs)
set(gca,'YTick',0:.5:1,'fontsize',fs)
ylim([0 1])
set(gca,'fontsize',fs)


subplot(122), cla %'Position',[left bottom1 width height]);
boxplot(allTimes(algsKept,1:100,1)'...
    ,'labels',{'FAQ','U','QCV','PATH'}...
    ,'labelorientation','inline'...
    ,'color','k','symbol','k+','plotstyle','compact'); %...
ylabel('time (seconds)','fontsize',fs)
set(gca,'fontsize',fs)
...
if savestuff==1
    wh=[4 2]*1.5;
    figName=[rootDir, '/figs/chemicalConnectome'];
