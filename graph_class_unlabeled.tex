...
\subsection{Quadratic assignment problem approach} % (fold)
...
% \end{align}
% The Frank-Wolfe (FW) algorithm is a successive linear programming algorithm specifically designed to solve quadratic programming problems with linear constraints \cite{??}.  Here, the problem is not quadratic in $Q$, rather, it is a 4th order polynomial function of $Q$.  Thus, 
FW will not (in general) achieve the global optimal.  
% Regardless, it proceeds as follows:
...
That the objective function is not necessarily convex is clear by virtue of computing the Hessian of the argument in \eqref{eq:a} with respect to $Q$:
\begin{align}
...
\clearpage
\appendix
\section{assignment problem solutions} % (fold)
\label{sec:assignment_problem_solutions}

% section assignment_problem_solutions (end)

\subsection{Linear assignment problem solution} % (fold)
