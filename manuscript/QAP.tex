...
\begin{align}
		\text{(rQAP) } \quad &\underset{P}{\text{minimize}}  && - tr(B\T PAP\T) \label{eq:FAQ1}  \\
...
because the Hessian of its objective function, $f(P)= - tr(B\T PAP\T)$, is not necessarily positive definite:
\begin{align}
...
% both (i) a kind of projection descent algorithm
% called the 
% We therefore develop a nonlinear programing algorithm for approximately solving FAQ using multiple restarts.
   % Our approach can be thought of as a 
% Frank-Wolfe (FW) algorithm, which is also 
% and (ii) a successive linear programming algorithm.  
% FW was originally designed for solving quadratic problems with linear constraints \cite{Frank1956}, though it later became used more generally for nonlinear programming problems  \cite{Bradley1977}.  Specifically, it 
% \texttt{FW} can be used to solve optimization problems of the following form:
...
% \end{align}
% \end{subequations}
% where $\mc{S} \subset \Real^d$ is a polyhedral set (a set described by linear constraints) and the function $f: \mc{S} \to \Real$ is continuously differentiable.  Generally, \texttt{FW} consists of the following steps.  Given an initial position, $x^{(0)}$, recurse the following two steps until some convergence criterion is met (e.g., computational budget).  First, find $\mt{x}^{(i)}$, the minimum of the first-order Taylor series approximation of $f$ around $x^{(i)}$ such that  $\mt{x}^{(i)} \in \mc{S}$.  Second, let $x^{(i+1)}$ minimize $f(x)$ such that $x^{(i+1)}$ is on the line segment from $x^{(i)}$ to $\mt{x}^{(i)}$ in $\mc{S}$.  
...
% which is also a LAP.  %Thus, this completes one restart of \texttt{FAQ}.

% paragraph the_final_iteration (end)

% \textbf{Multiple restarts} We refer to multiple re-starts of \FAQ with subscripts; that is, the performance of \FAQm is the best result of $m$ pseudo-random re-starts of \texttt{FAQ}.  We continue restarting until either we converge to a known global solution or we exceed our computational budget. Note that \FAQ natively operates on matrices, which could correspond to either weighted or unweighted, directed or undirected graphs.  
...
% 		\FOR{$j=1,2,\ldots$}
% 		\STATE Do \FAQ until convergence
...
% results inspire us to consider when \FAQ achieves the absolute minimum. Because 
...
% For the electrical one, the mean number of restarts was  $<30$ times, meaning that \FAQ sometimes found the optimal solution. 
...
% We also evaluated the performance of a number of other algorithms on these graph matching problems.  In particular, we acquired the `graphm' package, and ran all five algorithms, including the \texttt{PATH} algorithm, which outperformed the four from the package on these problems.  Specifically, the \texttt{PATH} algorithm got $60  (\pm 0.21) \%$  and $87  (\pm 0.02)\%$ accuracy on the chemical and electrical connectomes, respectively.  Interestingly, the performance of \texttt{PATH} is significantly worse for the chemical connectome, but significantly better for the electrical connectome.  This motivated us to consider multiple restarts of \FAQ for the electrical connectome.  Results are XXXXX.

 % of applying \FAQm to both $A_c$ and $A_e$ for $s=10$ times.  
...
% % % \end{align}
% Comparing this gradient to the gradient of the QAP objective function---Eq. \eqref{eq:grad}---one can see that when (i) $P^{(i)}$ is the identity matrix and (ii) both $A$ and $B$ are symmetric (for example, for undirected graphs), the two gradients are identical.  Thus, if QAP is initialized at the identity matrix and the graphs are undirected, the first permutation matrix---the output of \emph{Step 2} in our \FAQ algorithm---is identical to the LAP solution; although the line search will make $P^{(1)}$ not equal the LAP solution in general. %$ \neq Q_{LAP}$, in general.  %Moreoever, projecting a matrix onto the closest permutation matrix can be written as a LAP because of the following relationship:
% % \begin{align}
% % 	\argmin_{\PmcP} \norm{QA\T - I}_F^2 &= \argmin_{\PmcP} (QA\T-I)\T (QA\T-I) 
% % \nonumber\\ &=\argmin_{\PmcP} AQ\T QA\T -2QA\T - II = \argmin_{\PmcP}  -\langle Q, A\T \rangle  
% % \end{align}
% 
% % In the above simulation, the first iteration of \FAQ is essentially the only useful one.  Thus, we compare the performance of \texttt{BPI}$\circ$\texttt{LAP} (dark gray).  The performance of LAP and QAP are not statistically different for this simulation.  This suggests that for certain problems, LAP (which is $\mc{O}(n^3)$) is both an efficient and useful approximation to solving $\mc{NP}$-hard graph matching problems. We were unable to find a model for simple graphs in which multiple iterations of \FAQ improved performance over LAP. %We confirm this intuition by substituting QAP with LAP in the above simulations (black line).  As depicted in the above figures, this intuition is consistent with the numerical results. In other words, while naively one might implement an algorithm with exponential time complexity, LAP, which is only quadratic time complexity, will often suffice.
% 
% 
