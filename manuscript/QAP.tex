...
\begin{align}
	...
	&\argmin_{\PmcP}  -tr (P AB\T) = \argmin_{\PmcP}  -\langle (PA)\T, B \rangle = \label{eq:2c} \\
...
\end{align}
\end{subequations}
where the equality in on the second to last line defines the Euclidean inner product, $\langle X,Y\rangle = \sum_{i} x_i y_i$, and the last line follows because $P$ is a permutation matrix.  
