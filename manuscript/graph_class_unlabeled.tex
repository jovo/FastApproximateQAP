...
Let a graph be a 4-tuple consisting of a set of (i) vertices, (ii) edges, (iii) vertex labels, and (iv)  edge attributes. %, such that $\GG=(\VV,\EE,\LL,\Aa): \Omega \mapsto \mc{G}=(\mc{V},\mc{E},\mc{L},\mc{A})$.  Therefore, one can expand the joint distribution: $F_{\GG,\YY}=F_{\VV,\EE,\LL,\Aa, \YY}$.  
...
\subsection{Trying to use the labels} % (fold)
\label{sub:trying_to_use_the_labels}


Deciding the try to use the labels to classify

% subsection trying_to_use_the_labels (end)

\subsubsection{Graph Matching} % (fold)
