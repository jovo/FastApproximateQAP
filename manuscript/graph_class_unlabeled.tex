...
Let a graph be a 4-tuple consisting of a set of (i) $n_V$ vertices, (ii) $n_E$ edges, (iii) $n_V$ vertex labels, and (iv) $n_E$ edge attributes. %, such that $\GG=(\VV,\EE,\LL,\Aa): \Omega \mapsto \mc{G}=(\mc{V},\mc{E},\mc{L},\mc{A})$.  Therefore, one can expand the joint distribution: $F_{\GG,\YY}=F_{\VV,\EE,\LL,\Aa, \YY}$.  
...
\end{align}
where $\LL: \Omega \mapsto \mc{L}$ is a random vertex labeling, and $\Aa: \Omega \mapsto \mc{A} \subseteq \Real^{V \times V}$.  Within a class, therefore, there are two sources of variability, $\LL$ and $\Aa$.  

\subsection{Random Vertex Labelings} % (fold)
\label{sub:random_vertex_labelings}

 Let $Q \in \mc{Q}$ be a permutation matrix.  A graph can then be considered a set, $G=\{QAQ\T : Q \in \mc{Q}\}$.  The permutation matrix $Q$ essentially defines a vertex labeling.  Classical graph isomorphism deals with determining whether a pair of graphs are the same, given that only the adjacency matrices are observed, but not the permutation matrices.  To cast this problem into the above statistical framework, consider the following two-class classification problem.  As above, we observe $\{\GG_i,\YY_i\}_{i \in [n]} \iid F_{\GG,\YY}$.  However, the class-specific adjacency matrices are deterministic.  In other words, $F_{\Aa,\YY}$ is a Bernoulli random variable, where $F_{\Aa | \YY=0} \neq F_{\Aa | \YY=1}$, but each $F_{\Aa | \YY}$ is a point-mass. Thus, the joint graph/class distribution simplifies and factorizes: $F_{\GG,\YY}=F_{\LL | \YY} F_{\YY}$.  The task in this scenario is to find a vertex labeling for an unclassified graph.  If the unclassified graph is isomorphic to any graph in a class, then it is isomorphic to all graphs in that class, and it is not isomorphic to any graph in the other class.  Thus, one can proceed by trying to solve the graph isomorphism problem for the unclassified graph, relative to any graph with each class. 

% subsection random_vertex_labelings (end)

\subsection{Random Adjacency Matrices} % (fold)
\label{sub:random_adjacency_matrices}




% subsection random_adjacency_matrices (end)
